...
class QDFRCell(nn.Module):
    def __init__(self, n_hidden=10):
        ...
        self.register_buffer('l1_min_max', torch.zeros(2))
        self.l1_min_max[0] = -1.0 
        self.l1_min_max[1] = 1.0
    ...
    def forward(self, x, prev_output):
        ...
        q_mask = Quantize(self.mask, self.l1_min_max[0], self.l1_min_max[1])
        vec_x = torch.matmul(x, q_mask)
        q_vec_x = Quantize(vec_x, self.l1_min_max[0], self.l1_min_max[1])
        bias = Quantize(0.2 * prev_output, self.in_min_max[0], self.l1_min_max[1] - self.l1_min_max[0])
        output = self.act(q_vec_x + prev_output - bias)
        output = Quantize(output, self.in_min_max[0], self.l1_min_max[1] - self.l1_min_max[0])
        return output
