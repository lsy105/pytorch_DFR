...
#pre_trained = torch.load("db20_6.pch")
pre_trained = torch.load("Qdb20_6.pch")
teacher = torch.load("db20_6.pch").to(device)
teacher.eval()
#pre_trained = torch.load("Qmodel84_9.pch")
net.load_state_dict(pre_trained.state_dict(), strict=False)
optimizer = optim.SGD(net.parameters(), lr=lr, momentum=0.99, weight_decay=0.0, nesterov=True)
...
criterion = KDLoss(W=W, alpha=0.9, T=2)
...
temp_Q.Quantize(W, minval=0.0, maxval=2.0, row=1, col=hx1.shape[1])
"""
#quantize input
...
for name, weight in net.named_parameters():
    if "fc1" in name:
        ...
        temp_Q.Quantize(W, minval=-1.0, maxval=1.0, row=W.shape[0], col=W.shape[1])
    else:
        temp_Q = QParams(name)
        W = weight.detach().cpu().numpy()
