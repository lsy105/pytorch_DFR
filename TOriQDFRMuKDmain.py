...
# NARMA30 dataset
import scipy.io
x = scipy.io.loadmat('./Kian_data/recieved1_10db_4_4.mat')
...
#create system
net = TOriQDFRSystem(n_hidden=node_size, n_fc=n_fc).float().to(device)
T_net = TOriFloatDFRSystem(n_hidden=node_size, n_fc=n_fc).float().to(device)
...
criterion = KDLoss(W=W, alpha=0.1, T=10)
MSE = nn.MSELoss()
...
for epoch in range(epochs):
    ...
    for idx, data in enumerate(trainloader):
        ...
        output, med = net(x, hx)
        T_output, med_ref = T_net(x, T_hx)
        loss = criterion(output, y.long(), T_output.detach()) + 0.3 * MSE(med, med_ref.detach())
