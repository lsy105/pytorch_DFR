...
criterion = KDLoss(W=W, alpha=0.3, T=20)
...
def Eval(testloader, net, hx):
    ...
    W = np.array(save_x)
    temp_Q.Quantize(W, minval=-1, maxval=1.0, row=W.shape[0], col=W.shape[1], num_bits=16)
...
for name, weight in net.named_parameters():
    ...
    if "fc1.weight" in name:
        ...
        temp_Q.Quantize(W, row=W.shape[0], col=W.shape[1], num_bits=16)
        ...
        temp_Q.Quantize(W, minval= -0.5, maxval = 0.5, row=W.shape[0], col=W.shape[1], num_bits=16)
