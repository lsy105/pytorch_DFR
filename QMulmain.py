...
# Data loader
train_data = Dataset(x_train, y_train, 1, True)
test_data = Dataset(x_test, y_test, 1)
...
in_q = torch.quantization.FakeQuantize(observer=torch.quantization.MovingAverageMinMaxObserver,
                                       quant_min=0, quant_max=255, averaging_constant=0.01,
                                       dtype=torch.quint8, reduce_range=False)
for idx, data in enumerate(trainloader):
    x, y = data
    x = x.view(-1, 1).float().to(device)
    in_q(x)
print(in_q.scale, in_q.zero_point)
