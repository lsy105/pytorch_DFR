...
import numpy as np
from Dataset import Dataset, TwoDataset 
...
x_10 = scipy.io.loadmat('./Kian_data/recieved1_10db_4_4.mat')
y = scipy.io.loadmat('./Kian_data/target.mat')
x_data = x['recieved1']
x_10_data = x_10['recieved1']
y_data = y['target']

x_train = np.asarray(x_data[0:1000], dtype=float)
x_10_train = np.asarray(x_10_data[0:1000], dtype=float)
...
x_10_train = np.reshape(x_10_train, (len(x_10_train)))
x_test = np.reshape(x_test, (len(x_test)))
# Data loader
train_data = TwoDataset(x_train, x_10_train, y_train, sequence_length)
...
#create system
net = NewFloatDFRSystem(n_hidden=node_size, n_fc=n_fc).float().to(device)
T_net = NewFloatDFRSystem(n_hidden=node_size, n_fc=n_fc).float().to(device)
...
criterion = KDLoss(W=W, alpha=0.2, T=20)
...
def Eval(testloader, net):
    ...
    with torch.no_grad():
        for data in testloader:
            ...
            x = x.float().to(device)
            y = y.float().to(device)
            hx = torch.zeros(x.size(0), 1).to(device)
            optimizer.zero_grad()
            output = net(x, hx)
...
for epoch in range(epochs):
    ...
    for idx, data in enumerate(trainloader):
        ...
        y = y.float().to(device)
        hx = torch.zeros(x.size(0), 1).to(device)
        T_hx = torch.zeros(x.size(0), 1).to(device)
        optimizer.zero_grad()
        output = net(x, hx)
        T_output = T_net(x1, T_hx) 
