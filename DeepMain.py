...
x_test -= mean
x_test /= std
print(mean, std, np.max(np.abs(x_train)), np.max(np.abs(x_test)))
#parameters
batch_size = 1
grad_batch = 1 
epochs = 100
...
#create system
net = DeepFloatDFRSystem(ResidualBlock, num_blocks=1,  n_in=1, n_hidden=node_size).float().to(device)
...
for epoch in range(epochs):
    print("epoch:", epoch)
    hx = [torch.zeros(node_size).to(device) for _ in range(3)]
    running_loss = 0.0
    count, count1 = 0, 0
