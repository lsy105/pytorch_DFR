...
#pre_trained = torch.load("DFRfloatModel0.pch")
pre_trained = torch.load("ImageDFRfloat93.pch")
...
criterion = KDLoss(W=W, alpha=0.0, T=1)
...
def Eval(testloader, net, hx):
    ...
    W = np.array(save_x)
    temp_Q.Quantize(W, minval=-0.45, maxval=3.55, row=W.shape[0], col=W.shape[1], num_bits=10)
...
teacher1 = torch.load("RNNseqmodel29.pch")
teacher1.eval()
...
for epoch in range(epochs):
    ...
    for idx, data in enumerate(trainloader):
        ...
        t1_hx = [torch.zeros(batch_size, node_size).to(device) for _ in range(6)]
        t2_hx = [torch.zeros(node_size).to(device) for _ in range(num_DFR)]
        x, y = data
        #x = x.view(x.shape[0], -1).float().to(device)
        ...
        y = y.view(-1).long().to(device)
        optimizer.zero_grad()
        t_output = teacher1(x, t1_hx)
        #t_output = 0.5 * t1_output + 0.5 * t1_output 
        output = net(x, hx) 
...
torch.save(net, "QKDseqmodel" + str(epochs) + ".pch")
...
for name, weight in net.named_parameters():
    ...
    if "fc1.weight" in name:
        ...
        temp_Q.Quantize(W, row=W.shape[0], col=W.shape[1], num_bits=10)
        ...
        #temp_Q.GetFloat(W, row=W.shape[0], col=W.shape[1])
        temp_Q.Quantize(W, row=W.shape[0], col=W.shape[1], minval=-0.5, maxval=0.5, num_bits=10)
